...
// ── IndexedDB（2s 超时 + localStorage 降级） ──
    ...
    db = await Promise.race([
      new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = (e) => {
          ...
          if (e.oldVersion < 2) d.createObjectStore('settings', { keyPath: 'key' });
        };
        req.onsuccess = () => resolve(req.result);
        req.onerror = () => reject(req.error);
        req.onblocked = () => req.result?.close();
      }),
...
function saveRecord(record) {
  if (!dbOk) {
    const saved = JSON.parse(localStorage.getItem('cyber_records') || '[]');
    record.id = Date.now(); saved.push(record);
    localStorage.setItem('cyber_records', JSON.stringify(saved.slice(-50)));
...
function deleteRecord(id) {
  if (!dbOk) {
    const saved = JSON.parse(localStorage.getItem('cyber_records') || '[]');
    localStorage.setItem('cyber_records', JSON.stringify(saved.filter(r => r.id !== id)));
    return Promise.resolve();
  }
  return new Promise((resolve) => {
    const tx = db.transaction('records', 'readwrite');
...
function dbGet(key) {
  if (!dbOk) return Promise.resolve(localStorage.getItem(`cyber_${key}`) || null);
  return new Promise((resolve) => {
...
function dbSet(key, value) {
  if (!dbOk) { localStorage.setItem(`cyber_${key}`, value); return Promise.resolve(); }
  return new Promise((resolve) => {
...
function updateTestBtn() { $('#test-api-btn').disabled = !_apiKey; }

async function loadApiKey() {
  _apiKey = (await dbGet('deepseek_api_key')) || '';
  if (_apiKey) $('#apikey-input').value = _apiKey;
  updateTestBtn();
}

async function setApiKey(key) { _apiKey = key; await dbSet('deepseek_api_key', key); updateTestBtn(); }
function getApiKey() { return _apiKey; }

// ── 游戏状态 ──
const state = { name: '', origin: '', day: 1, hp: 100, credits: 50, rep: 50, hack: 50, alive: true, log: [] };
...
// ── 事件池 ──
const events = [
  { id:'fixer_job', title:'中间人找上门', desc:'一个叫"黑手"的中间人找到你，说有单生意：某企业需要有人从竞争对手服务器里偷数据。报酬丰厚，但风险不小。',
    choices:[
  ...
  { id:'street_doc', title:'地下诊所的义体医生', desc:'你在巷子里遇到一个义体医生。他说可以给你装一个二手义体——能提升反应速度，但设备来源不太干净。',
    choices:[
      { text:'安装（花费20信用点）', check:s=>s.credits>=20, effect:{credits:-20,hp:20,hack:10}, result:'义体装上了！虽然偶尔会有奇怪的卡顿，但确实让你更强了。' },
      { text:'砍价到10信用点', check:s=>s.credits>=10, effect:{credits:-10,hp:10}, result:'医生翻了个白眼，但还是成交了。' },
      { text:'走开', effect:{}, result:'你摇了摇头走开了，二手义体听起来就不靠谱。' }] },
  { id:'gang', title:'帮派火并', desc:'你路过桥口区时撞上了两帮派在交火。流弹擦着你的耳朵飞过。你可以趁机捡点掉落的装备，或者赶紧离开。',
    choices:[
  ...
  { id:'netrunner', title:'暗网悬赏', desc:'你在暗网上看到一个匿名悬赏：破解一个企业防火墙。赏金极高，但你知道这是在找死——企业网安不是闹着玩的。',
    choices:[
  ...
  { id:'market', title:'黑市交易', desc:'地下黑市今天格外热闹。有人卖二手赛博义体，有人兜售被窃的企业数据。你的信用点刚好够买点什么。',
    choices:[
      { text:'买黑客工具（-15信用）', check:s=>s.credits>=15, effect:{credits:-15,hack:15}, result:'你入手了一套最新的破解工具包，手感不错。' },
      { text:'买医疗补给（-15信用）', check:s=>s.credits>=15, effect:{credits:-15,hp:20}, result:'你囤了一些医疗纳米机器人和止痛剂。' },
      { text:'逛逛不买东西', effect:{}, result:'你逛了一圈，什么都没买就离开了。' }] },
  { id:'media', title:'被盯上了', desc:'你发现自己的照片出现在了一个本地地下新闻网站上，标题是《城市新晋赛博佣兵》——有人在关注你的一举一动。',
    choices:[
  ...
  { id:'corpo_raid', title:'公司突袭', desc:'半夜你被一阵嘈杂声吵醒——荒坂的安保小队正在对你所在的大楼进行"安全排查"。你只有几分钟时间决定怎么办。',
    choices:[
  ...
  { id:'ai_shard', title:'被遗弃的AI碎片', desc:'你在数据废料中发现了一个被分割的AI意识碎片。它用冰冷的合成声音对你说话了。"帮我重组完整，我会报答你。"',
    choices:[
...
要求：事件符合赛博朋克世界观，与玩家状态有呼应。2-3个选项，属性影响-25到+25。严格返回纯JSON（不要markdown代码块）：
{"title":"事件标题","desc":"事件叙述","choices":[{"text":"选项","effect":{"hp":0,"credits":0,"rep":0,"hack":0},"result":"结果描述"}]}`;

  const res = await fetch('https://api.deepseek.com/v1/chat/completions',{
    method:'POST',
    headers:{'Content-Type':'application/json','Authorization':`Bearer ${getApiKey()}`},
    body:JSON.stringify({model:'deepseek-chat',messages:[{role:'user',content:prompt}],temperature:0.9,max_tokens:800}),
  });
...
function startGame() {
  ...
  const o = origins[selectedOrigin];
  state.hp = 100 + o.hp; state.credits = 50 + o.credits; state.rep = 50 + o.rep; state.hack = 50 + o.hack;
  state.day = 1; state.alive = true; state.log = [];
  showScreen('game-screen');
  addLog(`<b>${state.name}</b>，欢迎来到夜之城。`, 'system');
  addLog(o.desc, 'system');
  updateUI(); newDay();
}
...
function renderChoices(evt) {
  ...
  evt.choices.forEach(ch => {
    ...
    if (ch.check && !ch.check(state)) { btn.disabled=true; btn.style.opacity='0.3'; btn.style.cursor='not-allowed'; }
...
function resolveEvent(choice, evt) {
  let eff = {...choice.effect}, res = choice.result;
  if (choice.check && choice.failEffect && !choice.check(state)) { eff = {...choice.failEffect}; res = choice.failResult || res; }
  applyEffect(eff);
  addLog(res, eff.hp<0 || (choice.failEffect && choice.failEffect.hp) ? 'bad' : 'good');
  if (Math.random()<0.3 && state.alive) bonusEvent();
  $('#choices').innerHTML = '';
...
function bonusEvent() {
  ...
  addLog(evt.text, (evt.effect.hp||0)<0?'bad':'good');
}

function applyEffect(eff) {
  state.hp = Math.max(0, Math.min(100, state.hp + (eff.hp||0)));
  state.credits = Math.max(0, Math.min(100, state.credits + (eff.credits||0)));
  state.rep = Math.max(0, Math.min(100, state.rep + (eff.rep||0)));
  state.hack = Math.max(0, Math.min(100, state.hack + (eff.hack||0)));
  updateUI();
  if (state.hp <= 0) { state.alive = false; gameOver('你的生命值降至零点。赛博空间里又多了一个被遗忘的灵魂。'); }
  if (state.credits <= 0) { state.alive = false; gameOver('信用点耗尽，你无法在城市里生存下去。你沦为了街头流浪者。'); }
}

function updateUI() {
  $('#hp-bar').style.width = state.hp+'%'; $('#hp-text').textContent = state.hp;
  $('#credits-bar').style.width = state.credits+'%'; $('#credits-text').textContent = state.credits;
  $('#rep-bar').style.width = state.rep+'%'; $('#rep-text').textContent = state.rep;
  $('#hack-bar').style.width = state.hack+'%'; $('#hack-text').textContent = state.hack;
}
...
async function gameOver(epitaph) {
  ...
  $('#end-stats').innerHTML = `
    ...
    <div class="stat-row"><span>出身</span><span>${{corpo:'公司狗',hacker:'网络黑客',nomad:'废土游民'}[state.origin]}</span></div>
  ...
  await saveRecord({ name:state.name, origin:state.origin, originLabel:{corpo:'公司狗',hacker:'网络黑客',nomad:'废土游民'}[state.origin], day:state.day, hp:state.hp, credits:state.credits, rep:state.rep, hack:state.hack, score, death:epitaph, time:new Date().toLocaleString('zh-CN') });
}

function showScreen(id) { $$('.screen').forEach(s => s.classList.remove('active')); $(`#${id}`).classList.add('active'); }
function restartGame() {
  state.name=''; state.origin=''; state.day=1; state.hp=100; state.credits=50; state.rep=50; state.hack=50;
  state.alive=true; state.log=[]; selectedOrigin=null;
  $$('.origin-btn').forEach(b => b.classList.remove('selected'));
  $('#name-input').value=''; $('#start-btn').disabled=true;
  $('#log').innerHTML=''; $('#choices').innerHTML=''; $('#next-btn').classList.add('hidden');
...
async function boot() {
  ...
  $('#test-api-btn').addEventListener('click', async () => {
    const btn = $('#test-api-btn'), status = $('#api-status');
    btn.textContent = '⏳ 测试中...'; btn.classList.add('testing'); status.className = 'api-status hidden';
    try {
      const res = await fetch('https://api.deepseek.com/v1/chat/completions', {
        method:'POST',
        headers:{'Content-Type':'application/json','Authorization':`Bearer ${_apiKey}`},
        body:JSON.stringify({model:'deepseek-chat',messages:[{role:'user',content:'回复一个字：通'}],max_tokens:4}),
      });
      if (res.ok) { status.textContent='✅ 连接成功！DeepSeek API 可用'; status.className='api-status success'; }
      else { const err = await res.json().catch(()=>({})); status.textContent=`❌ API 错误 (${res.status})：${err.error?.message||'未知错误'}`; status.className='api-status error'; }
    } catch (e) {
      status.textContent = `❌ 网络失败：${e.message.includes('Failed to fetch')?'CORS 跨域拦截，浏览器无法直接调 DeepSeek API':e.message}`;
      status.className = 'api-status error';
    }
    btn.textContent='🔍 测试连接'; btn.classList.remove('testing');
  });

  // 出身选择
  $$('.origin-btn').forEach(btn => { btn.addEventListener('click', () => {
    $$('.origin-btn').forEach(b => b.classList.remove('selected'));
    btn.classList.add('selected'); selectedOrigin = btn.dataset.origin;
  ...
  $('#next-btn').addEventListener('click', () => {
    if (!state.alive) return;
    state.day++;
    if (state.day % 5 === 0) { applyEffect({credits:-5}); addLog('你支付了这个周期的基础生活费用（-5 信用点）。', 'system'); if (!state.alive) return; }
    applyEffect({hp:-2}); newDay();
  });
  ...
  async function loadHistory() {
    ...
    if (records.length === 0) { list.innerHTML=''; empty.classList.remove('hidden'); }
    else {
      empty.classList.add('hidden');
      list.innerHTML = records.map(r => `
        <div class="history-card">
          <div><div class="card-name">${r.name}</div><div class="card-date">${r.time}</div></div>
          <div class="card-score">${r.score}</div>
          <div class="card-stats"><span>🏷️ ${r.originLabel}</span><span>📆 ${r.day}天</span><span>❤️ ${r.hp}</span><span>₿ ${r.credits}</span><span>⭐ ${r.rep}</span></div>
  ...
  $('#history-btn-start').addEventListener('click', loadHistory);
